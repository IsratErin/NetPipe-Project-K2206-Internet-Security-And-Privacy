...
public class HandshakeCertificate {
    ...
    public HandshakeCertificate(byte[] certbytes) throws CertificateException {
       ...
       // this.myCertificate = (X509Certificate)certificateFactory.generateCertificate(in);
    }
     ...
     */
     ...
     public String getCN() {
    	    String cn = myCertificate.getSubjectX500Principal().getName().split(",")[1].split("=")[1];
    ...
    public static String hexStringToString2(String hex) {
        hex = hex.startsWith("#") ? hex.substring(1) : hex;
        StringBuilder sboutput = new StringBuilder();
        int ourdecimalValue;
        for (int i = 0; i < hex.length(); i += 2) {
        ...
        return sboutput.substring(cchar);
    }
